...
//  ExpenseTracker
//
//  Created by Erkan on 19.09.2023.
//
...
class ExpensesDetailController: UIViewController, UITextFieldDelegate, FSCalendarDelegate, FSCalendarDataSource {
    ...
    var visualEffectView: UIVisualEffectView? // Görünüm efekti katmanı
    ...
    @objc func gelirTapped(){
        ...
        if tiklandi == TrackerEnum.Gider.rawValue{
            ...
            kategoriLabelButton.setTitleColor(.lightGray, for: .normal)
            kategoriLabelButton.setTitleColor(UIColor.lightGray.withAlphaComponent(0.2) , for: .highlighted)
    ...
    @objc func giderTapped(){
      ...
      //  giderButton.tintColor = UIColor(red: 176/255, green: 217/255, blue: 162/255, alpha: 1)
        giderButton.backgroundColor = UIColor(red: 255/255, green: 159/255, blue: 159/255, alpha: 0.6)
       // giderButton.layer.borderColor = UIColor.red.cgColor
        ...
        ///
        self.tiklandi = "Gider"
        kategoriArray = ["Sağlık", "Araba Masrafları","Ev Harcamaları", "Evcil Hayvan", "Ulaşım", "Dışarıda Yemek", "Spor", "Giyim", "Alışvreriş", "Tatil", "Eğitim", "İş", "Diğer"]
...
//UIColor(red: 45/255, green: 151/255, blue: 45/255, alpha: 1)
   ...
   //     calendar.anchor(top: nil, bottom: nil, leading: nil, trailing: nil, paddingTop: 0, paddingBottom: 0, paddingLeft: 0, paddingRight: 0, width: 300, height: 400)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        backItem.title = category.localized()
   ...
   //     barButtons()
    ...
    //    backView.addSubview(hesapLabel)
       ...
       // tabBarController?.tabBar.isHidden = true
        ...
        kategoriLabelButton.anchor(top: kategoriLabel.topAnchor, bottom: kategoriLabel.bottomAnchor, leading: kategoriLabel.trailingAnchor, trailing: view.trailingAnchor, paddingTop: 0, paddingBottom: 0, paddingLeft: 10, paddingRight: 0, width: 0, height: 0)
    ...
    //   backView.backgroundColor = .red
        backView.anchor(top: gelirButton.bottomAnchor, bottom: view.lastBaselineAnchor, leading: view.leadingAnchor, trailing: view.trailingAnchor, paddingTop: 10, paddingBottom: 15, paddingLeft: -5, paddingRight: 5, width: 0, height: 0)
    ...
    func guncelleTrack(){
        ...
        do {
            ...
            if let objectToUpdate = result.first {
                ...
                objectToUpdate.date = selectedDate
                objectToUpdate.note = notTextField.text
                if tiklandi == TrackerEnum.Gider.rawValue{
                    //newTrack.setValue(-self.tutar, forKey: "privatemoney")
                    if asilSayi < 0{
                        objectToUpdate.privatemoney = asilSayi
                    }else{
                        objectToUpdate.privatemoney = -asilSayi
                    }
    ...
    //    print(uuid)
        ...
        do {
            let fiterTask = try context.fetch(fetchRequest)
            for task in fiterTask as! [NSManagedObject] {
                guard let categoryName = task.value(forKey: "category") as? String else{return}
                guard let categoryMoney = task.value(forKey: "privatemoney") as? Float else{return}
                guard let categoryNote = task.value(forKey: "note") as? String else{return}
                guard let categoryDate = task.value(forKey: "date") as? Date else{return}
                
                selectedDate = categoryDate
                let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "dd/MM/yyyy"
                let formattedDate = dateFormatter.string(from: categoryDate)
                
                selectedCategory = categoryName
                print(selectedCategory)
                
                asilSayi = categoryMoney
                
            print("Tutar : \(asilSayi)")
                
                if asilSayi < 0{
                    gelirButton.backgroundColor = UIColor.lightGray.withAlphaComponent(0.3)
                  //  giderButton.tintColor = UIColor(red: 176/255, green: 217/255, blue: 162/255, alpha: 1)
                    giderButton.backgroundColor = UIColor(red: 255/255, green: 159/255, blue: 159/255, alpha: 0.6)
                    self.tiklandi = TrackerEnum.Gider.rawValue
                    moneyText.setTitle("\(-asilSayi)", for: .normal)
                    kategoriArray = ["Sağlık", "Araba Masrafları","Ev Harcamaları", "Evcil Hayvan", "Ulaşım", "Dışarıda Yemek", "Spor", "Giyim", "Alışvreriş", "Tatil", "Eğitim", "İş", "Diğer"]
                    kategoriArrayLanguages = ["Health", "Car Expenses", "Household", "Pet Care", "Transport", "Dining Out", "Sports", "Clothing", "Shopping", "Vacation", "Education", "Work", "Other"]
                    ...
                    gelirButton.backgroundColor = UIColor(red: 179/255, green: 255/255, blue: 179/255, alpha: 0.6)
                    giderButton.backgroundColor = UIColor.lightGray.withAlphaComponent(0.3)
                    self.tiklandi = TrackerEnum.Gelir.rawValue
                    moneyText.setTitle("\(asilSayi)", for: .normal)
            ...
            kategoriLabelButton.setTitle(selectedCategory.localized(), for: .normal)
            notTextField.text = categoryNote
            tarihTextButton.setTitle("\(formattedDate)", for: .normal)
            }
        }catch{
...
extension ExpensesDetailController: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
      ...
      //  let text = kategoriArray[indexPath.row]
       ...
       // kategoriArray = ["Sağlık", "Araba Masrafları","Ev Harcamaları", "Evcil Hayvan", "Ulaşım", "Dışarıda Yemek", "Spor", "Giyim", "Alışvreriş"]
        ...
        }else if kategoriArray[indexPath.row] == "Alışvreriş"{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
       ...
       if let selectedCell = collectionView.cellForItem(at: indexPath) {
           ...
           kategoriLabelButton.setTitleColor(.black, for: .normal)
           categoryView.isHidden = true
